...
export async function handleAppleAuth(request, env) {
  ...
  let user = await findUserByAppleId(env.DB, appleUser.apple_user_id);
  let isNew = false;
  ...
  if (!user) {
    ...
    await initCredits(env.DB, user.id);
    isNew = true;
  } else if (user_name && !user.name) {
    await updateUser(env.DB, user.id, { name: user_name });
    user.name = user_name;
  }
...
export async function handleGoogleAuth(request, env) {
  ...
  let user = await findUserByGoogleId(env.DB, googleUser.google_user_id);
  let isNew = false;
