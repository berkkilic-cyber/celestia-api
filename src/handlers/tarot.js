...
export async function handleTarot(request, env) {
  ...
  for (let i = 0; i < cards.length; i++) {
    if (!cards[i]?.name?.trim())
      return { error: `cards[${i}] must have a non-empty name`, status: 400 };
    if (typeof cards[i].reversed !== "boolean")
      return { error: `cards[${i}].reversed must be a boolean`, status: 400 };
  }
